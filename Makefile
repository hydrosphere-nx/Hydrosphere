...
ifeq ($(strip $(HYDROSPHERE)),)
$(error "Please set HYDROSPHERE in your environment. export HYDROSPHERE=<path to>/hydrosphere")
endif

all: deploy

install-helpers:
	meson setup helper build/helper --cross helper/dummy-cross
	ninja -C build/helper install

build-hydrosphere-aarch64: install-helpers install-builtins install-module-runtime
	meson setup libhydrosphere build/aarch64/libhydrosphere --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch64-nx --buildtype=debugoptimized
	ninja -C build/aarch64/libhydrosphere

build-hydrosphere-aarch32: install-helpers install-builtins install-module-runtime
	meson setup libhydrosphere build/aarch32/libhydrosphere --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch32-nx --buildtype=debugoptimized
	ninja -C build/aarch32/libhydrosphere

build-builtins-aarch64: install-helpers
	meson setup rt-builtins build/aarch64/rt-builtins --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch64-nx
	ninja -C build/aarch64/rt-builtins

build-builtins-aarch32: install-helpers
	meson setup rt-builtins build/aarch32/rt-builtins --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch32-nx
	ninja -C build/aarch32/rt-builtins

build-module-runtime-aarch64: install-helpers
	meson setup module-runtime build/aarch64/module-runtime --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch64-nx
	ninja -C build/aarch64/module-runtime

build-module-runtime-aarch32: install-helpers
	meson setup module-runtime build/aarch32/module-runtime --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch32-nx
	ninja -C build/aarch32/module-runtime

build-rtld-aarch64: install-helpers build-builtins-aarch64
	meson setup rtld build/aarch64/rtld --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch64-nx --buildtype=release
	ninja -C build/aarch64/rtld

build-rtld-aarch32: install-helpers build-builtins-aarch32
	meson setup rtld build/aarch32/rtld --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch32-nx --buildtype=release
	ninja -C build/aarch32/rtld

build-application-template-aarch64: install-helpers
	meson setup application-template build/aarch64/application-template --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch64-nx
	ninja -C build/aarch64/application-template

build-application-template-aarch32: install-helpers
	meson setup application-template build/aarch32/application-template --cross-file $(HYDROSPHERE)/usr/share/meson/cross/aarch32-nx
...
prepare-launch-aarch64: deploy build-application-template-aarch64
	...
	cp $(HYDROSPHERE)/usr/lib/aarch64/rtld.nso output/aarch64/rtld
	cp $(HYDROSPHERE)/usr/lib/aarch64/libhydrosphere.nso output/aarch64/sdk
...
prepare-launch-aarch32: deploy build-application-template-aarch32
	...
	cp $(HYDROSPHERE)/usr/lib/aarch32/rtld.nso output/aarch32/rtld
	cp $(HYDROSPHERE)/usr/lib/aarch32/libhydrosphere.nso output/aarch32/sdk
